...
__buf_get(ptr, idx, offset) {
    ...
    bit_shift = (idx & offset_mask) << (6 - offset);
    word = ptr[word_index];
...
_buf_memmove_u8(dst, src, count) { return __buf_memmove(&_buf_get_u8, &_buf_set_u8, dst, src, count); }
_buf_memmove_u16(dst, src, count) { return __buf_memmove(&_buf_get_u16, &_buf_set_u16, dst, src, count); }
_buf_memmove_u32(dst, src, count) { return __buf_memmove(&_buf_get_u32, &_buf_set_u32, dst, src, count); }
...
_buf_memset_u8(dst, val, count) { return __buf_memset(&_buf_set_u8, dst, val, count); }
_buf_memset_u16(dst, val, count) { return __buf_memset(&_buf_set_u16, dst, val, count); }
_buf_memset_u32(dst, val, count) { return __buf_memset(&_buf_set_u32, dst, val, count); }
